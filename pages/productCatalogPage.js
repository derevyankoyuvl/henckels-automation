...
class ProductCatalogPage {
  constructor() {
    ...
    this.breadcrumbs = locate('[data-sentry-element="BreadcrumbList"]').as(
      "Breadcrumbs"
    )
    this.categoryHeader = locate('[data-testid="category-header"]').as(
      "Category Header"
    )

    // Product item locators
    this.productLink = locate('[data-sentry-element="Link"]').as("Product Link")
    this.productTitle = locate('[data-testid="product-title"]').as(
      "Product Title"
    )
    this.productPrice = locate('[data-testid="product-price"]').as(
      "Product Price"
    )
    this.productImage = locate('[data-sentry-element="Image"]').as(
      "Product Image"
    )
    this.productRating = locate('[itemprop="aggregateRating"]').as(
      "Product Rating"
    )
    this.addToCartBtn = locate(
      "button[data-sid^='product_add_to_cart_item']"
    ).as("Add To Cart Button")

    // Filter section locators
    this.filterBtn = locate('[data-sid="filter_filter"]').as("Filter Button")
    this.filterPanel = locate('[data-sentry-element="SheetContent"]').as(
      "Filter Panel"
    )
    this.closeFilterBtn = locate('[data-sid="close-button"]').as(
      "Close Filter Button"
    )
    this.clearAllFiltersBtn = locate('[data-sentry-element="Button"]').as(
      "Clear All Filters Button"
    )
    this.applyFiltersBtn = locate('[data-sid="filter_apply"]').as(
      "Apply Filters Button"
    )

    // Sorting locators
    this.sortOptionsBtn = locate('[data-sid="filter_sort"]').as(
      "Sort Options Button"
    )
    this.sortByRecommended = locate('[data-sid="sort_recommended"]').as(
      "Sort By Recommended"
    )
    this.sortByPriceLowToHigh = locate(
      '[data-sid="sort_price_low_to_high"]'
    ).as("Sort By Price Low To High")
    this.sortByPriceHighToLow = locate(
      '[data-sid="sort_price_high_to_low"]'
    ).as("Sort By Price High To Low")
    this.sortByAvailability = locate('[data-sid="sort_availability"]').as(
      "Sort By Availability"
    )

    // Pagination locators
    this.loadMoreBtn = locate('[aria-label="Load more products"]').as(
      "Load More Button"
    )
    ...
    this.filterOptionByName = (filterName) =>
      locate(`[data-testid="filter-${filterName.toLowerCase()}"]`).as(
        `${filterName} Filter Option`
      )

    this.sortOptionByType = (sortType) =>
      locate(
        `[data-sid="sort_${sortType.toLowerCase().replace(/\s+/g, "_")}"]`
      ).as(`Sort By ${sortType}`)
  }
  ...
  verifyBreadcrumbs() {
    I.waitForElement(this.breadcrumbs)
    I.seeElement(this.breadcrumbs)
  }

  // Search and Results Verification
  async verifySearchReturnsResults(searchValue) {
    // Wait for search results to load
    I.wait(2)

    // Assert that we have search result cards
    ...
    for (let i = 1; i <= cardElements; i++) {
      ...
      if (!containsSearchTerm) {
        throw new Error(
          `Card ${i} should contain "${searchValue}" but contains: ${cardText}`
        )
      }
    }
  }

  async verifySearchReturnsNoResults() {
    // Wait for search results to load
    I.wait(2)
    ...
    if (cardElements > 0) {
      throw new Error(
        `Expected no search results but found ${cardElements} results`
      )
    }
  ...
  addProductToCart(productIndex) {
    const addToCartButton = locate(this.addToCartBtn)
      .at(productIndex + 1)
      .as(`Add To Cart Button ${productIndex}`)
